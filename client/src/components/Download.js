...
class Download extends Component {
    ...
    onClickHandler = () => {
        ...
        axios
            .post(
                "/api/db/file/" + this.props.match.params.fileId,
                data,
                config
            )
            .then(res => {
    ...
    render = () => {
                    ...
                    <li className="flex-item">
                        <div>
                            <div className="note ">
                                ...
                                <p>
                                    Expires:{" "}
                                    {this.state.document.expirationDate}
                                </p>
                            </div>
                            <p className="center">
                                <input
        ...
        const expiredTemplate = (
            <div>
                <h1 className="center header-description header-text">
                    {/* {this.state.fileStatusMessage} */}
                    Your File Has Expired or Reached its Download Limit
                </h1>
                <ul className="flex-container">
                    <li className="flex-item">
                        <div>
        ...
        const returnTemplate = () => {
            if (this.state.document.fileStatus === undefined) {
                return <p />;
            } else if (this.state.document.fileStatus.length === 0) {
